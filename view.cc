...
void view_t::read( istream& ins )
{
	...
	// Read in commands
	else if( cmd == "coi" )    { ins >> coi;          seen_coi = true; }
	else if( cmd == "eye" )    { ins >> eye;          seen_eye = true; }
	else if( cmd == "vwidth" ) { ins >> viewport_u;   seen_vwidth = true; }
	else if( cmd == "aspect" ) { ins >> aspect_ratio; seen_aspect = true; }
    ...
    if(!( seen_coi && seen_eye && seen_vwidth && seen_aspect && seen_xres &&
				seen_focal && seen_up )){
...
ostream& operator << ( ostream& os, const view_t& v )
{
    ...
    os << "xres x yres: "    << v.num_x_pixels << " x " << v.num_y_pixels << "\n";
